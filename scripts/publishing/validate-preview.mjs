...
export async function validatePreviewSource() {
...
export async function validatePreviewArtifact() {
  const {promotion,result} = await validatePreviewSource();
  const dist = join(root,"dist");
  const headersPath = join(dist,"_headers");
  const existingHeaders = await readFile(headersPath,"utf8");
  const previewHeaders = existingHeaders.replace(
    /^\/\*\s*\n/,
    "/*\n  X-Robots-Tag: noindex, nofollow\n  Cache-Control: no-store\n"
  );
  await writeFile(headersPath,previewHeaders);
  ...
  if (!html.length || html.some((page) => !/name="robots" content="noindex,nofollow"/.test(page))) {
    throw new Error("Every preview HTML page must be noindex,nofollow");
  }
  if (html.some((page) =>
  ...
  for (const article of promotion.articles) {
    ...
    if (!page.includes("Preview candidate — not published")) {
      throw new Error(`Preview marker missing from article: ${article.id}`);
    }
  ...
  if (!/Disallow: \//.test(robots)
    ...
    throw new Error("Preview robots, feed exclusion, or response-cache contract failed");
  }
  return result;
}
...
const result = mode === "artifact"
  ? await validatePreviewArtifact()
  : (await validatePreviewSource()).result;
console.log(`Boho News candidate preview validation passed (${result.articleCount} articles; ${mode}).`);
