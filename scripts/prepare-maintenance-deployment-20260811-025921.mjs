...
const slugs=["doe-nuclear-cleanup-end-state-contracts-cost-growth-gao-2026","federal-prison-population-first-step-act-statistics-2024","smithsonian-pulverer-japanese-books-digital-catalog","gao-opm-eight-priority-recommendations-2026","gao-overlapping-cybersecurity-reporting-rules-2026","nsf-four-year-phd-industry-research-pilot-2026"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/investigations/","/crime-justice/","/culture/","/politics/","/technology/","/us/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
