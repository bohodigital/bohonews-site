...
const slugs=["hybrid-electric-saab-340-flight-above-30000-feet","nasa-sweet-15-wing-failed-127-percent-design-load","sec-regulation-e-delivery-paper-opt-out-proposal-2026","italy-growth-05-percent-inflation-29-imf-2026","education-department-title-vi-disparate-impact-provisions-2026","who-kala-azar-14-day-treatment-guidelines-2026"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/technology/","/us/","/business/","/world/","/politics/","/health-science/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
