...
const slugs=["ncaa-womens-lacrosse-penalty-rules-proposed-2026","fda-tregzi-regulatory-t-cell-therapy-approval-2026","national-recording-registry-2026-25-selections","cambodia-growth-inflation-imf-2026-outlook","401k-spousal-consent-withdrawals-gao-2026","gao-pentagon-clean-audit-approach-2028-risks"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/sports/","/health-science/","/culture/","/world/","/business/","/investigations/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
