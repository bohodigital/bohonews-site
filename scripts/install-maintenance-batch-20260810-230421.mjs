...
const mediaRoot="/media/newsroom/2026/08/manual-20260810-230421";
...
const articleDefs=JSON.parse(readFileSync(join(root,"scripts/maintenance-batch-20260810-230421.json"),"utf8"));
...
const replaceableCandidateSlugs=[
  "gao-six-agencies-covered-telecom-equipment-searches-2026",
  "federal-sentences-within-guideline-range-59-percent-2026",
  "nist-adds-52000-chemical-fingerprints-mass-spectra-2026",
  "census-household-trends-tables-136000-homes-march-2026",
  "noaa-urban-air-chemistry-particle-formation-2026",
  "imf-gambia-22-million-disbursement-growth-forecast-2026"
];
const candidateTail=promotion.articles.slice(-6).map(({slug})=>slug);
const supersededCandidateSlugs=[
  "gao-100-broadband-programs-no-national-strategy-2026",
  "federal-sentences-within-guideline-range-59-percent-2026",
  "nist-adds-52000-chemical-fingerprints-mass-spectra-2026",
  "census-household-trends-tables-136000-homes-march-2026",
  "noaa-urban-air-chemistry-particle-formation-2026",
...
const replaceable=[replaceableCandidateSlugs,supersededCandidateSlugs].some((slate)=>candidateTail.join("\n")===slate.join("\n"));
if(promotion.releaseState==="candidate"&&replaceable){
  promotion.articles.splice(-6);
  promotion.mediaRights.splice(-18);
...
promotion.inputHashes={sourceItems:digest(articleDefs.map(({sources})=>sources)),events:digest(articleDefs.map(({slug})=>`event-${slug}`)),claims:digest(articleDefs.map(({facts})=>facts)),articles:digest(newArticles),approvals:digest({mode:"owner-authorized-maintenance",date:"2026-08-10"}),corrections:digest([]),mediaRights:digest(newRights),releaseRecords:digest(promotion.releaseRecords),publicationIntents:digest(articleDefs.map(({slug})=>({slug,intent:"manual-maintenance-release"})))};
