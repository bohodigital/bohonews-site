...
const slugs=["coast-guard-cleanup-liabilities-448-million-gao-2026","tribal-law-enforcement-violent-offenses-down-11-percent-2023","library-congress-gil-evans-manuscripts-unrecorded-scores","cbo-president-2027-budget-defense-nondefense-spending","imf-tanzania-444-million-final-program-reviews-2026","mlb-draft-2026-first-day-135-picks-position-breakdown"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/investigations/","/crime-justice/","/culture/","/politics/","/world/","/sports/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
