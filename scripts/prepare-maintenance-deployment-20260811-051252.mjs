...
const slugs=["gao-federal-program-effectiveness-three-step-test-2026","nih-oriva-human-based-research-office-2026","smithsonian-caterpillar-2-3-million-stem-40-communities","imf-ireland-growth-inflation-outlook-2026","us-rental-homeowner-vacancy-rates-second-quarter-2026","ncaa-division-i-commercial-uniform-patches-august-2026"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/investigations/","/health-science/","/culture/","/world/","/business/","/sports/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
