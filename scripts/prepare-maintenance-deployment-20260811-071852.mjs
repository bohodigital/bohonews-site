...
const slugs=["state-overseas-housing-standards-35-years-gao","fed-july-2026-bank-lending-standards-demand","nih-human-cancer-models-665-25-tumor-types","smithsonian-bronze-bison-national-mall-exhibitions-2026","ncaa-womens-flag-football-championship-funding-vote-2026","who-global-health-days-evaluation-108-observances"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/investigations/","/crime-justice/","/politics/","/weather-climate/","/technology/","/us/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
