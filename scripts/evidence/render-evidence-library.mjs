...
function documentActions(record, compact = false) {
  const local = record.ok ? `<a class="button button--primary" href="/evidence/files/${record.sha256}.pdf">${compact ? "Open PDF" : "Read preserved PDF"}</a>` : "";
...
function documentCard(record) {
  ...
  return `<article class="document-card" data-type="${escapeHtml(record.metadata.documentType.id)}" data-search="${escapeHtml(documentSearch(record))}">
    ...
    <details class="provenance"><summary>Integrity and provenance</summary><dl>${sourceChecks(record)}<div><dt>Preserved copy</dt><dd>${escapeHtml(dateLabel(record.firstPreservedAt ?? record.retrievedAt) ?? "")}</dd><dt>SHA-256</dt><dd><code>${escapeHtml(record.sha256)}</code></dd><dt>File size</dt><dd>${(record.bytes / 1024 / 1024).toFixed(2)} MB</dd></div></dl></details>
    <div class="used-by"><h3>Reporting that uses this source</h3><ul>${usedBy}</ul></div>
  </article>`;
}

export function renderEvidenceLibrary(records, stories, { interlochenOnly = false } = {}) {
  const filteredRecords = interlochenOnly
    ...
    : records;
  const recordIds = new Set(filteredRecords.map((record) => record.sha256));
  const filteredStories = stories.map((story) => ({
    ...story,
    records: story.records.filter((record) => recordIds.has(record.sha256)),
    suggested: story.suggested.filter((record) => recordIds.has(record.sha256))
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${escapeHtml(title)} | Boho News</title>
<style>
...
<main><section class="hero"><p class="eyebrow">Sources behind the reporting</p><h1>${escapeHtml(title)}</h1><p class="intro">Primary documents and source material used in Boho News reporting. Browse by story or document type, open preserved copies, and inspect when each source link was last checked.</p><div class="summary"><div><strong>${filteredRecords.length}</strong><span>source documents</span></div><div><strong>${filteredStories.length}</strong><span>reporting collections</span></div><div><strong>${totalPages.toLocaleString()}</strong><span>pages</span></div><div><strong>${totalChecks}</strong><span>source links checked</span></div></div></section>
