...
const slugs=["federal-tax-fraud-median-loss-546562-fy2025","metro-bachelors-degree-rate-378-percent-acs-2024","human-speech-neurons-grammar-context-nih-study-2026","mauritius-growth-28-percent-debt-86-imf-2026","from-these-lands-smithsonian-600-objects-2029","ncaa-womens-soccer-college-cup-second-weekend-december-2026"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/crime-justice/","/us/","/health-science/","/world/","/culture/","/sports/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
