...
const slugs=["gao-six-agencies-covered-telecom-equipment-searches-2026","federal-sentences-within-guideline-range-59-percent-2026","nist-adds-52000-chemical-fingerprints-mass-spectra-2026","census-household-trends-tables-136000-homes-march-2026","noaa-urban-air-chemistry-particle-formation-2026","imf-gambia-22-million-disbursement-growth-forecast-2026"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/investigations/","/crime-justice/","/technology/","/us/","/weather-climate/","/world/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
