...
const slugs=["federal-supervised-release-rate-fell-82-percent-fy2025","gao-federal-strategic-reviews-requirements-2026","cbo-social-security-one-time-wealth-tax-75-year-solvency","prescribed-burns-sequoia-survival-nasa-study-2026","nist-looping-laser-mixes-3d-printed-alloys-2026","public-pension-assets-649-trillion-census-2025"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/crime-justice/","/investigations/","/politics/","/weather-climate/","/technology/","/business/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
