...
const slugs=["main-street-lending-251-loans-outstanding-after-maturity","credit-card-financial-fraud-718-federal-sentences-fy2025","smithsonian-in-pursuit-250-objects-exhibition-2026","prediabetes-lifestyle-intervention-lowered-multimorbidity-risk","beijing-2027-world-athletics-qualification-standards","federal-excise-tax-revenue-106-billion-2025"];
...
const articleEvidence=articles.map((article)=>({articleId:article.id,bodySha256:sha256(Buffer.from(article.body)),dek:article.dek,headline:article.headline,route:new URL(article.canonicalUrl).pathname}));const routes=["/","/rss.xml","/sitemap.xml","/news-sitemap.xml","/search/","/robots.txt","/investigations/","/crime-justice/","/culture/","/health-science/","/sports/","/politics/",...(phase==="activation"?["/games/"]:[]),...articleEvidence.map(({route})=>route)];
