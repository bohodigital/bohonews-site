...
export const GET: APIRoute = ({params}) => {
  const items = articles.filter((a) => a.section === params.section && a.distribution.rss && !a.fixture).map((a) => `<item><title>${escape(a.headline)}</title><link>${a.canonicalUrl}</link><guid>${a.canonicalUrl}</guid><pubDate>${new Date(a.publishedAt).toUTCString()}</pubDate></item>`).join("");
