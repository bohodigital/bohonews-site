...
import { articles, sections } from "../lib/news";
const base = "https://bohonews.com";
export const GET: APIRoute = () => {
  const paths = ["/","/search/","/about/","/corrections/","/editorial-standards/","/contact/","/privacy/","/terms/",...sections.map(([id]) => `/${id}/`),...articles.filter((a) => !a.fixture && a.search.index && a.retractionState === "current").map((a) => `/articles/${a.slug}/`)];
