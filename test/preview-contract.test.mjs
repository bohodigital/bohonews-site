...
const fixturePath = (relative) =>
  new URL(`./fixtures/mcp-v2/${relative}`, import.meta.url);
const readJson = async (relative) =>
  JSON.parse(await readFile(fixturePath(relative), "utf8"));
...
async function materializePublicFixture(mediaRights) {
  ...
  const sourceByRole = new Map(
    privateRights.derivatives.map((item) => [item.role, item.path]),
  );
...
test("authoritative candidate validates only in the disconnected preview lane", async () => {
  const candidate = await readJson(
    "examples/json/09-candidate-promotion-package.v2.1.1.json",
  );
  const manifest = await readJson(
    "examples/json/10-candidate-release-manifest.v2.1.1.json",
  );
  const schema = JSON.parse(
    await readFile(
      new URL(
        "../schemas/public-news-promotion-package.v2.1.1.schema.json",
        import.meta.url,
      ),
      "utf8",
    ),
  ...
  assert.equal(
    validatePublicState(candidate, manifest, schema, {
      preview: true,
      publicRoot,
    }).articleCount,
    1,
  ...
  assert.throws(
    () =>
      validatePublicState(invented, manifest, schema, {
        preview: true,
        publicRoot,
      }),
...
test("authoritative final package binds canonical-first-public time and release marker", async () => {
  const promotion = await readJson(
    "examples/json/17-final-promotion-package.v2.1.1.json",
  );
  const manifest = await readJson(
    "examples/json/18-final-release-manifest.v2.1.1.json",
  );
  const marker = await readJson(
    "examples/json/19-public-release-marker.v1.1.json",
  );
  const schema = JSON.parse(
    await readFile(
      new URL(
  ...
  assert.equal(
    validatePublicState(promotion, manifest, schema, { publicRoot })
      .articleCount,
    1,
  );
  assert.equal(
    validateReleaseMarker(marker, promotion, manifest, { publicRoot })
      .releaseId,
    marker.releaseId,
  );
  assert.equal(
...
test("preview source contract suppresses timestamps, indexing, analytics, and TradingView", async () => {
  const [
    ...
    readFile(
      new URL("../scripts/publishing/validate-preview.mjs", import.meta.url),
      "utf8",
    ),
    ...
    readFile(
      new URL("../src/pages/news-sitemap.xml.ts", import.meta.url),
      "utf8",
    ),
  ]);
  ...
  assert.match(
    article,
    /candidateUnpublishedEnabled \? undefined : article\.publishedAt/,
  );
  assert.match(layout, /!candidateUnpublished/);
  assert.match(layout, /const productionArtifact = !candidateUnpublished/);
  assert.match(
    layout,
    /!productionArtifact && \(candidateUnpublished \|\| noindex\)/,
  );
